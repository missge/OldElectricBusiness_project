<template>
	<div class="DetailsPage">
		<div class="DP_nav">
			...
			<div class="DP_nav_rt">
				<img class="DP_icon1" width="24px" height="20px" src="./img/DP_icon1.png" @click="dump('Ehome')">
				<img width="17px" height="22px" src="./img/DP_icon2.png" @click="dump('Home')">
				<img class="DP_icon3" width="23px" height="20px" src="./img/DP_icon3.png" @click="dump('PersonalCenter')"> 
			</div>
		</div>
		<div class="DP_cont1 white_bg">
			<div class="DP_swiper">
				<mt-swipe :auto="2500" :show-indicators="true"  ref="swipeWrapper">
			...
			<div class="DP_introduction">
				...
				<p class="DP_red">{{datalist.goodDescr}}</p>
			</div>
		</div>
	...
	<!-- 	<div class="white_bg DP_Sales_box">
		...
		<div class="holidayPrompt" v-show="markedWords != ''">
			<b>提示：</b>{{markedWords}}
		</div>
		...
		<div class="DP_list_Spec  white_bg flex spec_address" @click="showCity">
		...
		<div class="DP_list_Spec  white_bg" @click="goMallList(datalist.merId)">
			<span class="DP_list_Spec_title">进入查看更多相关商品</span>
			<img class="icon_jt" width="9px" height="10px" src="./img/jt.png">
		</div>
		...
		<!-- 	<div class="DP_servers_top">
			...
			<!-- <div class="DP_servers_bot">
				<div class="wide_btn_blue" @click="goMallList(datalist.merId)">进入店铺》</div>
			</div>
		</div> -->
		...
		<div class="DP_shopCard ">
			<div class="DP_shopCard_lf">
				...
				<div class="DP_shop_two" @click="ShoppingCard()">
					...
					<p>购物车</p>
					<mt-badge size="small" type="error" v-if="amount!=0">{{amount}}</mt-badge>
			...
			<div class="change_spec"  v-if="clicked==true" >
				<div class="change_header">
					<div class="change_header_img">
						<!-- <img :src='datalist.photoUrl[0] || picture' width="100%" height="100%"> -->
						<img :src="picture!=null ? picture:datalist.photoUrl[0] " width="100%" height="100%">
					</div>
				...
				<ul class="change_spec_content">
					<li v-for="(items, index) in datalist.mallGoodAttrInfoDTO">
						<div class="change_width">
							...
							<!-- 	<span  :class="map.get(items.fieldValue)==items.strArray[index]?'empty_bluebtn':'empty_whitebtn'"  @click="searchAttrFn(items.fieldValue,items.strArray[index],datalist.mallGoodAttrInfoDTO.length)" v-for="(strArray,index) in items.strArray">{{strArray}}
								</span> -->
								<span  :class="nowchange==items.strArray[index] || map.get(items.fieldValue)==items.strArray[index]?'empty_bluebtn':'empty_whitebtn'"  @click="searchAttrFn(items.fieldValue,items.strArray[index],datalist.mallGoodAttrInfoDTO.length)" v-for="(strArray,index) in items.strArray">{{strArray}}
								</span>
			...
			<div class="Shipping_address" v-if="viewcity==true">
				<div class="change_width">
					<span v-if="datalist.supportAreaList!=null&&datalist.supportAreaList.length>0">
						<div class="shipping_title">仅配送以下地区:</div>
...
export default{
	...
	methods:{
			...
			that.$http.post(url,data,{emulateJSON:true}).then(
	      		function (res){
      				if(!res.data.ret){
      					...
      					that.markedWords=res.data.markedWords
      				}
					else{
	    ...
	    serachAmount(){
	    	var url = this.$store.state.localHostUrl +'/searchMallCartAmount.json'
	    	var data= qs.stringify({mallId:getCookie('mallId')})
			var that = this
			that.$http.post(url,data,{emulateJSON:true}).then(
	      		function (res){
      				if(!res.data.ret){
		...
		addShop(buyflag){	
			...
			if(this.priceFlag==0&&this.datalist.mallGoodAttrInfoDTO!=null){
				this.$toast('小主，您还木有选择规格嘞!')
			}
			if(buyflag==0){
  ...
  // 			document.addEventListener("touchmove",mo,false);//禁止页面滑动
	 ...
	 //  	   let mo=function(e){e.preventDefault()}
		...
		goMallList(merId){
			this.$router.push({path:'/MallList',query:{merId:merId,searchFlag:2}})
		},
		getUrlKey (name) {
...
<style>
.spec_address .DP_list_Spec_title {
	width:70px;
    flex: 0 0 70px;
    text-align: left;
}
.spec_address .flex_rt{
	    flex: 1;
    text-align: right;
    padding-right: 25px;
    line-height: 48px;
...
.DP_nav{height: 1.8rem;line-height: 1.8rem;display: flex;padding-left: 10px; background: #fff;margin-bottom:3px;}
.DP_nav_lf{flex: 1;font-size: 0.75rem;color:#434343;overflow: hidden; text-overflow: ellipsis; white-space: nowrap;}
.DP_nav_rt{flex:0 0 150px;width:150px;}	
.DP_nav_rt>img{vertical-align: middle;    margin-right: 10px;}
.DP_icon1{padding-left: 15px;}
.DP_swiper{height: 360px;width: 100%;margin:0 auto;padding: 10px 0 ;}
...
.DP_red{padding:12px 0 16px 0; font-size: 0.7rem;color: #E60012;line-height:18px}
...
.DP_servers_top{height: auto;line-height: 20px;padding:20px 0;}
...
.DP_goods_title{    font-size: 0.75rem;
    padding: 13px 5px;
    background: #fff;}
.DP_goods_box{width: 100%;}
.DP_goods_box>li{padding-bottom:5px;}
.DP_goods_box img{max-width: 100%;width: 100%}
...
.DP_buy{	flex: 1;background: #F08632;text-align: center;}
.DP_shop_one,.DP_shop_two{flex:1;text-align: center;position: relative;}
...
.model_box>.model{    position: absolute;
  ...
  .DP_shop_two .mint-badge.is-error{
  	position: absolute;
    top: 2px;
    right: 4px;
  }
  .DP_goods_introduce img{display: block;}
 .holidayPrompt{
	  	padding: 10px 10px;
	    background: #FCECEF;
	    border-top: 1px solid #F81D31;
	    border-bottom: 1px solid #F81D31;
	    color: #E60011;
	    margin-bottom: 5px;
	    line-height: 17px;
	    font-size: 0.6rem;
